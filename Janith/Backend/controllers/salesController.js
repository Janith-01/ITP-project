...
//janith implemented
export const getSelectedFieldsFromAllSales = async (req, res) => {
    try {
      const saleData = await Sale.find({}, { productName: 1, category: 1, quantitySold: 1, unitPrice: 1, dateSold: 1 }).lean();
      if (!saleData || saleData.length === 0) {
        return res.status(404).json({ msg: "Sales data not found" });
      }
  
      
      const selectedSalesData = saleData.map((sale) => ({
      ...
      res.status(200).json(selectedSalesData);
    } catch (error) {
      res.status(500).json({ error: error });
    }
  };