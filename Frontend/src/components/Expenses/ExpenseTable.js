...
const ExpenseTable = () => {
  ...
  const fetchExpenseData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8083/api/totalExpense"
      );
  ...
  const fetchExtraExpenses = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8083/api/transactions/get-extraexpenses"
      );
  ...
  const handleDeleteClick = async (id) => {
    try {
      await axios.delete(
        `http://localhost:8083/api/transactions/delete-extraexpense/${id}`
      );
      fetchExtraExpenses();
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:8083/api/transactions/update-extraexpense/${_id}`,
        updateFields
      );
      fetchExtraExpenses();
      handleClose();
